...
VISUAL_PAGE_REVIEW_SCHEMA = "arc.paper.visual_page_review.v1"
VISUAL_PAGE_TERMINAL_SCHEMA = "arc.paper.visual_page_terminal.v1"
...
class PdftoppmFullPageRenderer:
    ...
    def render(self, pdf_bytes: bytes) -> tuple[RenderedPDFPage, ...]:
        ...
        with tempfile.TemporaryDirectory(prefix="arc-paper-pdf-render-") as directory:
            root = Path(directory)
    ...
    def review(
        ...
        *,
        ...
        markdown_ref = _publish_input(
            context,
            f"paper-visual/markdown/{primary.source.artifact_digest}",
            markdown_bytes,
            "text/markdown",
        )
        ...
        manifest_ref = _publish_json_input(
            context,
            f"paper-visual/manifests/{primary.document_digest}",
            manifest_document,
        )
        ...
        for page in rendered_pages:
            ...
            page_ref = _publish_input(
                context,
                (
                    f"paper-visual/pages/{pdf_validator.source.artifact_digest}/"
            ...
            terminal_id = f"paper-visual/terminal/{request.task_id}"
            try:
...
def _math_manifest(spans: tuple[MathSpan, ...]) -> dict[str, Any]:
    return {
        "schema_version": "arc.paper.math_span_manifest.v1",
        "spans": [
            {
