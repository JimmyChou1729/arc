...
class _SelectedDocument:
    ...
    def stable_identity(self) -> tuple[str, ...]:
        if self.source_kind == "arxiv":
            return (self.source_kind, *self.arxiv_ids)
        ...
        for item in selected:
            ...
            if previous is None:
                ...
                merged[key] = replace(
                    ...
                    arxiv_ids=tuple(
                        sorted(
                            set(previous.arxiv_ids).union(item.arxiv_ids),
                            key=str.casefold,
                        )
                    ),
                )
...
def _display_title(
    ...
    for page in document.pages:
        for line in page.text.splitlines():
            ...
            if title:
                return title
    if selected.arxiv_ids:
        return ", ".join(selected.arxiv_ids)
    representation = selected.representation
    return (
