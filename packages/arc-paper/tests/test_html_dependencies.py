...
from arc_paper.html_acquisition import (
    GatedPinnedTransport,
    HttpxRequestGateTransport,
    ReferenceMaterialCacheHTMLStorage,
    _AcquisitionProvenance,
)
from arc_paper.html_dependencies import acquire_html_dependencies
...
def _provider(
    ...
    **kwargs,
    ...
    kwargs.setdefault("dependency_resolver", lambda _host: ("8.8.8.8",))
    kwargs.setdefault(
        "dependency_transport_factory",
        lambda client, request_gate: HttpxRequestGateTransport(client, request_gate),
    )
...
def test_gated_pinned_transport_preserves_acf_validated_addresses() -> None:
    ...
    class Transport:
        def fetch(self, url: str, **kwargs: object) -> object:
            ...
            return type("Response", (), {"status": 200, "headers": {}, "body": b""})()

    result = GatedPinnedTransport(Gate(), Transport()).fetch(
    ...
    assert received == {
        "gated": True,
        "url": "https://arxiv.org/html/0911.3380",
        "timeout_seconds": 60,
        "maximum_bytes": 1024,
        "validated_addresses": ("8.8.8.8",),
    }
...
def test_acf_projection_matches_upstream_main_failure_golden(tmp_path: Path) -> None:
    """Golden generated by executing `git show upstream/main` legacy code."""

    body = b'<html><body><article class="ltx_document"><img src="missing.png"><img src="redirect.png"><img src="huge.png"><img src="write.png"></article></body></html>'

    def handler(request: httpx.Request) -> httpx.Response:
        url = str(request.url)
        if url == MAIN_URL:
            return _response(request, content=body)
        if url.endswith("missing.png"):
            return _response(request, status_code=404, content=b"missing", media_type="image/png")
        if url.endswith("redirect.png"):
            return _response(request, status_code=302, headers={"location": "https://evil.test/escaped.png"}, media_type="image/png")
        if url.endswith("huge.png"):
            return _response(request, headers={"content-length": "6"}, media_type="image/png")
        if url.endswith("write.png"):
            return _response(request, content=b"write", media_type="image/png")
        raise AssertionError(url)

    provider = _provider(tmp_path, handler, max_dependency_bytes=5)
    original_store = provider.resource_cache.store_resource

    def fail_write(payload: bytes, **kwargs: object):
        if payload == b"write":
            raise OSError("fixture cache write failure")
        return original_store(payload, **kwargs)

    provider.resource_cache.store_resource = fail_write
    bundle = provider.fetch_bundle(PAPER_ID)
    document = __import__("arc_paper").html_source_bundle_to_document(bundle)

    assert [item["error_code"] for item in document["dependencies"]] == [
        "html_dependency_fetch_failed",
        "html_dependency_redirect_invalid",
        "html_dependency_too_large",
        "html_dependency_cache_write_failed",
    ]
    assert [item["request_url"] for item in document["dependencies"]] == [
        "https://arxiv.org/html/missing.png",
        "https://arxiv.org/html/redirect.png",
        "https://arxiv.org/html/huge.png",
        "https://arxiv.org/html/write.png",
    ]
    assert document["dependencies"][0]["error_message"] == (
        "Client error '404 Not Found' for url 'https://arxiv.org/html/missing.png'\n"
        "For more information check: https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/404"
    )
    assert document["dependencies"][1]["error_message"] == (
        "remote redirect left the allowed provider boundary: https://evil.test/escaped.png"
    )
    assert document["dependencies"][2]["error_message"] == "remote response exceeds 5 bytes"
    assert document["dependencies"][3]["error_message"] == "fixture cache write failure"
    assert document["bundle_digest"] == "a2c7e507ba1149d02673665fd0a00a26003914016ffb44c7315417f306cb169c"


def test_query_assets_match_upstream_v2_golden_and_materialize_distinctly(tmp_path: Path) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        if str(request.url) == MAIN_URL:
            return _response(request, content=_article('<img src="plot.png?v=1"><img src="plot.png?v=2">'))
        return _response(request, content=str(request.url).encode(), media_type="image/png")

    service = ArcPaperService(cache_root=tmp_path, arxiv_html=_provider(tmp_path, handler))
    bundle = service.fetch_arxiv_html_bundle(PAPER_ID)
    assert bundle.bundle_digest == "b6eb2fc312901898f993655e17e89f480678281b013f79d6b3a08454cf989154"
    assert [item.availability for item in bundle.dependencies] == ["available", "available"]
    result = service.export_arxiv_html_acquisition(PAPER_ID, output_dir=tmp_path / "export")
    paths = [item["path"] for item in result["export"]["resources"]]
    assert len(paths) == len(set(paths)) == 2
    assert all(path.startswith("assets/arxiv.org/html/") for path in paths)


@pytest.mark.parametrize(
    ("body", "routes", "kwargs", "message", "digest"),
    [
        ('<img src="a.png"><img src="b.png">', {"a.png": (200, b"a")}, {"max_dependency_count": 1}, "ignored 1 dependency references beyond the limit of 1", "52160c4c791103442656e504f55fe6257adfb3999b204a416742d850aca2fa82"),
        ('<img srcset="a.png 1x, b.png 2x">', {}, {}, "srcset candidate selection is not supported by bundle schema v2", "61a022c8c484990576bc273f5148ff7ac787862ee5ae6f447502e3b1332078f9"),
        ('<img src="start.png">', {"start.png": (302, b"", "final.png"), "final.png": (404, b"")}, {}, "Client error '404 Not Found' for url 'https://arxiv.org/html/final.png'\nFor more information check: https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/404", "566568d107fa685d6d6e74d149f8216190f93997471fe815ec8d370f2722c2f7"),
    ],
)
def test_acf_projection_matches_upstream_main_warning_matrix(tmp_path: Path, body, routes, kwargs, message, digest) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        if str(request.url) == MAIN_URL:
            return _response(request, content=f'<article class="ltx_document">{body}</article>'.encode())
        name = str(request.url).rsplit("/", 1)[-1]
        status, content, *location = routes[name]
        return _response(request, status_code=status, content=content, headers=({"location": location[0]} if location else {}), media_type="image/png")

    bundle = _provider(tmp_path, handler, **kwargs).fetch_bundle(PAPER_ID)
    document = __import__("arc_paper").html_source_bundle_to_document(bundle)
    assert document["bundle_digest"] == digest
    assert document["warnings"][-1]["message"] == message


def test_storage_provenance_binds_cache_write_failure_to_request_url() -> None:
    class FailingCache:
        def store_resource(self, payload: bytes, *, media_type: str):
            raise OSError("write fixture")

    provenance = _AcquisitionProvenance()
    provenance.current_request_url = "https://arxiv.org/html/write.png"
    storage = ReferenceMaterialCacheHTMLStorage(None, FailingCache(), provenance)

    with pytest.raises(Exception) as error:
        storage.store_dependency(b"write", media_type="image/png")
    assert getattr(error.value, "code", "") == "html_dependency_cache_write_failed"
    assert provenance.storage_failures == {
        "https://arxiv.org/html/write.png": ["write fixture"]
    }


def test_dependency_acquisition_seam_receives_validated_exact_revision(
    tmp_path: Path,
) -> None:
    versioned = f"{PAPER_ID}v4"
    document_url = f"https://arxiv.org/html/{versioned}"
    payload = (
        "<html><head>"
        f'<base href="/html/{versioned}/assets/">'
        "</head><body>"
        ...
        '<article class="ltx_document"><img src="plot.png"></article>'
        "</body></html>"
    ).encode()
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if str(request.url) == document_url:
            return _response(request, content=payload)
        if str(request.url) == f"{document_url}/assets/plot.png":
            return _response(request, content=b"plot", media_type="image/png")
        raise AssertionError(str(request.url))

    def capture_acquisition(payload: bytes, **kwargs: object):
        observed["payload"] = payload
        observed.update(kwargs)
        return acquire_html_dependencies(payload, **kwargs)

    provider = _provider(
        tmp_path,
        handler,
        dependency_acquirer=capture_acquisition,
    )
    bundle = provider.fetch_bundle(versioned)

    primary = observed["primary"]
    assert observed["payload"] == payload
    assert primary == bundle.primary
    assert primary.origin.locator == document_url
    assert primary.origin.metadata["arxiv_version"] == "v4"
    assert observed["provider"] == "arxiv-html"
    assert observed["document_url"] == document_url
    assert observed["allowed_host"] == "arxiv.org"
    assert observed["resource_cache"] is provider.resource_cache
    assert bundle.bundle_digest


def test_dependency_acquisition_seam_preserves_v2_export_bytes_and_paths(
    tmp_path: Path,
) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if url == MAIN_URL:
            return _response(
                request,
                content=_article(
                    '<object data="figures/plot.svg"></object>'
        ...
        if url.endswith("/static/absolute.png"):
            return _response(request, content=png, media_type="image/png")
        raise AssertionError(url)

    def delegate_to_current_acquirer(payload: bytes, **kwargs: object):
        return acquire_html_dependencies(payload, **kwargs)

    before_root = tmp_path / "before"
    before_service = ArcPaperService(
        cache_root=before_root,
        arxiv_html=_provider(before_root, handler),
    )
    before_bundle = before_service.fetch_arxiv_html_bundle(PAPER_ID)
    before_export = before_service.export_arxiv_html_bundle(
        PAPER_ID,
        output_dir=tmp_path / "before-export",
    )

    after_root = tmp_path / "after"
    after_service = ArcPaperService(
        cache_root=after_root,
        arxiv_html=_provider(
            after_root,
            handler,
            dependency_acquirer=delegate_to_current_acquirer,
        ),
    )
    after_bundle = after_service.fetch_arxiv_html_bundle(PAPER_ID)
    after_export = after_service.export_arxiv_html_bundle(
        PAPER_ID,
        output_dir=tmp_path / "after-export",
    )

    assert after_bundle.bundle_digest == before_bundle.bundle_digest
    assert after_bundle.primary == before_bundle.primary
    assert after_bundle.dependencies == before_bundle.dependencies
    assert after_bundle.dependencies[2].attribute == "srcset"
    assert after_bundle.dependencies[2].availability == "unavailable"
    assert after_bundle.dependencies[2].error_code == (
        "html_dependency_srcset_unsupported"
    )
    assert after_service.repository.read_bytes(after_bundle.primary) == (
        before_service.repository.read_bytes(before_bundle.primary)
    )
    assert (Path(after_export["source"])).read_bytes() == (
        Path(before_export["source"])
    ).read_bytes()
    ...
    after_resources = resource_signature(after_export, tmp_path / "after-export")
    assert after_resources == resource_signature(
        before_export, tmp_path / "before-export"
    )
    assert after_resources == [
        {
            "ordinal": 0,
            "authored_target": "figures/plot.svg",
            "path": "figures/plot.svg",
            "artifact_digest": after_bundle.dependencies[0].artifact_digest,
            "media_type": "image/svg+xml",
            "size": len(svg),
        }
    ]
    assert "html_dependency_target_not_materializable" in {
        item["code"] for item in after_export["warnings"]
    }
    assert not (Path(after_export["source"]).parent / "static").exists()


def test_ar5iv_fallback_uses_the_dependency_acquisition_seam(
    tmp_path: Path,
) -> None:
    ...
    def arxiv_handler(request: httpx.Request) -> httpx.Response:
        assert str(request.url) == MAIN_URL
        return _response(request, status_code=404)

    def ar5iv_handler(request: httpx.Request) -> httpx.Response:
        if str(request.url) == ar5iv_url:
            return _response(request, content=_article('<img src="plot.png">'))
        if str(request.url) == f"https://ar5iv.labs.arxiv.org/html/plot.png":
            return _response(request, content=b"plot", media_type="image/png")
        raise AssertionError(str(request.url))
    ...
    arxiv = _provider(tmp_path, arxiv_handler)
    ar5iv = Ar5ivProvider(
        cache_root=tmp_path,
        client=httpx.Client(transport=httpx.MockTransport(ar5iv_handler)),
        request_gate=HostRequestGate(minimum_interval=0),
        dependency_acquirer=capture_acquisition,
    ...
    bundle = ArcPaperService(
        cache_root=tmp_path,
        arxiv_html=arxiv,
        ar5iv=ar5iv,
    ).fetch_arxiv_html_bundle(PAPER_ID)

    primary = observed["primary"]
    assert bundle.provider == "ar5iv"
    assert observed["document_url"] == ar5iv_url
    assert observed["allowed_host"] == "ar5iv.labs.arxiv.org"
    assert primary == bundle.primary
    assert primary.origin.provider == "ar5iv"


def test_acquisition_export_replays_sidecar_after_archive_without_legacy_bundle(
    tmp_path: Path,
) -> None:
    svg = b'<svg xmlns="http://www.w3.org/2000/svg"/>'
    png = b"\x89PNG\r\nfixture"
    ...
    provider = _provider(source_cache, handler)
    service = ArcPaperService(cache_root=source_cache, arxiv_html=provider)
    first = service.export_arxiv_html_acquisition(
        PAPER_ID,
        output_dir=tmp_path / "first-export",
    )

    first_manifest = json.loads(Path(first["manifest"]).read_text(encoding="utf-8"))
    assert first["sidecar_status"] == "replayed"
    assert first_manifest["schema_version"] == "ac.document.html_source_export.v1"
    assert [item["path"] for item in first_manifest["resources"]] == [
        "figures/plot.svg",
        "assets/arxiv.org/static/absolute.png",
    ]
    assert first_manifest["rewrites"] == [
        {
            "dependency_ordinal": 1,
            "element": "img",
            "attribute": "src",
            "authored_target": "/static/absolute.png",
            "materialized_target": "assets/arxiv.org/static/absolute.png",
        }
    ]
    assert b"assets/arxiv.org/static/absolute.png" in Path(first["source"]).read_bytes()
    ...
    replay_provider = _provider(
        ...
        lambda request: (_ for _ in ()).throw(
            AssertionError("sidecar replay must not access the network")
        ),
    )
    assert replay_provider.cache.remove("json", "arxiv-html-dependencies", PAPER_ID)
    replay = ArcPaperService(
        cache_root=target,
        arxiv_html=replay_provider,
    ).export_arxiv_html_acquisition(
        PAPER_ID,
        output_dir=tmp_path / "replay-export",
    )

    assert replay["sidecar_status"] == "replayed"
    assert Path(replay["source"]).read_bytes() == Path(first["source"]).read_bytes()
    assert json.loads(Path(replay["manifest"]).read_text(encoding="utf-8")) == (
        first_manifest
    )


def test_acquisition_export_marks_a_v2_only_cache_as_legacy_fallback(
    tmp_path: Path,
) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        url = str(request.url)
    ...
    provider = _provider(cache_root, handler)
    service = ArcPaperService(cache_root=cache_root, arxiv_html=provider)
    service.fetch_arxiv_html_bundle(PAPER_ID)
    assert provider.cache.remove("json", "arxiv-html-acquisition", PAPER_ID)

    fallback = service.export_arxiv_html_acquisition(
        PAPER_ID,
        output_dir=tmp_path / "legacy-export",
    )
    manifest = json.loads(Path(fallback["manifest"]).read_text(encoding="utf-8"))

    assert fallback["sidecar_status"] == "legacy_fallback"
    assert "assets/arxiv.org/static/absolute.png" not in {
        item["path"] for item in manifest["resources"]
    }
    assert "arc_html_acquisition_sidecar_missing" in {
        item["code"] for item in manifest["bundle"]["warnings"]
    }


def test_acquisition_export_output_error_keeps_healthy_sidecar(tmp_path: Path) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        if str(request.url) == MAIN_URL:
            return _response(request, content=_article('<img src="plot.png">'))
        return _response(request, content=b"plot", media_type="image/png")
    ...
    with pytest.raises(RuntimeError):
        service.export_arxiv_html_acquisition(PAPER_ID, output_dir=output_file)
    assert provider.cache.get_json("arxiv-html-acquisition", PAPER_ID) is not None

    replay = service.export_arxiv_html_acquisition(PAPER_ID, output_dir=tmp_path / "clean")
    assert replay["sidecar_status"] == "replayed"


def test_acquisition_export_corrupt_sidecar_repairs_with_refresh(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[bool] = []
    provider = _provider(tmp_path, lambda request: _response(request, content=_article('<img src="plot.png">')) if str(request.url) == MAIN_URL else _response(request, content=b"plot", media_type="image/png"))
    service = ArcPaperService(cache_root=tmp_path, arxiv_html=provider)
    service.fetch_arxiv_html_bundle(PAPER_ID)
    original = provider.cache.get_json
    corrupt = [True]
    def sidecar_get(namespace, key):
        if namespace == "arxiv-html-acquisition" and corrupt[0]:
            corrupt[0] = False
            from arc_paper.providers.remote_cache import RemoteCacheError
            raise RemoteCacheError("remote_cache_json_corrupt", "fixture")
        return original(namespace, key)
    provider.cache.get_json = sidecar_get
    fetched = service.fetch_arxiv_html_bundle
    monkeypatch.setattr(service, "fetch_arxiv_html_bundle", lambda paper_id, *, refresh=False: calls.append(refresh) or fetched(paper_id, refresh=refresh))
    service.export_arxiv_html_acquisition(PAPER_ID, output_dir=tmp_path / "out")
    assert calls == [True]
